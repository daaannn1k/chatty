...
export class NotificationUtils {
    ...
    socketService?.socket?.on('insert notification', (data, userToData) => {
      if(profile?._id === userToData.userTo) {
        ...
        if(type === 'notificationPage') {
          ...
          const mappedNotifications = this.mapNotificationDropdownItems(notifications, setNotificationsCount);
  ...
  static mapNotificationDropdownItems(notificationData, setNotificationsCount) {
    ...
    const count = sumBy(items, (item) => item.read === false ? 1 : 0);
    setNotificationsCount(count);

    return items;
  }

  static async markMessageAsRead (notificationId, notification, setNotificationDialogContent) {
